...
/**
 * OLD PRELOADER CODE - COMMENTED OUT FOR REFERENCE
 * 
 * This was the previous GSAP-based vault preloader with animated bills
 * and complex 3D animations. Kept for potential future use.
 * 
...
/**
 * NEW NETFLIX-STYLE PRELOADER
 * Animated letter reveal effect with brush and light animations
 */
export default function PreloaderGSAP({ onFinish = () => {} }) {
  const preloaderRef = useRef(null);

  useEffect(() => {
    // Wait 4 seconds for the animation to complete, then fade out
    const timer = setTimeout(() => {
      const preloader = preloaderRef.current;
      if (preloader) {
        preloader.classList.add("netflix-preloader--fade-out");
  ...
  return (
    <div id="preloader" className="netflix-preloader" ref={preloaderRef}>
      <div id="container" className="netflix-container">
        {/* Netflix Letter Animation - Letter "N" */}
        <netflixintro letter="N" className="netflix-letter">
          {/* Helper 1 - Left vertical bar */}
          ...
          {/* Helper 2 - Right vertical bar */}
          <div className="helper-2">
            <div className="effect-brush">
              {[...Array(31)].map((_, i) => (
                <span key={`fur-h2-${31 - i}`} className={`fur-${31 - i}`}></span>
              ))}
            </div>
          </div>

          {/* Helper 3 - Diagonal connector */}
          <div className="helper-3">
            <div className="effect-brush">
              {[...Array(31)].map((_, i) => (
                <span key={`fur-h3-${31 - i}`} className={`fur-${31 - i}`}></span>
              ))}
            </div>
          </div>

          {/* Helper 4 - Optional additional element */}
          <div className="helper-4">
            <div className="effect-brush">
              {[...Array(31)].map((_, i) => (
                <span key={`fur-h4-${31 - i}`} className={`fur-${31 - i}`}></span>
              ))}
            </div>
          </div>
        </netflixintro>
